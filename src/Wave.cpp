...
void Wave::update(const bool windowSizeChanged)
{
	...
	if (windowSizeChanged)
		m_spawnRanges = {
					...
					std::array{std::uniform_real_distribution(0.f, hgui::point(100_em).x), std::uniform_real_distribution(hgui::point(-10_em).y, 0.f)}
				};
}
...
void Wave::check_wave_status()
{
	...
	m_meteors.erase(std::ranges::remove_if(m_meteors, [](const auto& meteor) { return !meteor->is_alive(); }).begin(), m_meteors.end());
	if (m_bots.empty() && m_spawnedMeteors == std::div(m_waveNumber, 10ll).quot * 100 && m_meteors.empty())
	{
		m_waveNumber++;
		generate_wave();
		update();
...
void Wave::spawn_meteors()
{
	static std::random_device rd;
	static std::mt19937 gen(rd());
	static std::uniform_int_distribution randomSize(0, 6);
	m_meteors.emplace_back(std::make_shared<Meteor>(get_random_position(true), randomSize(gen), hgui::vec2(0, 10)));
	m_spawnedMeteors++;
	if (m_spawnedMeteors < std::div(m_waveNumber, 10ll).quot * 100)
		m_tempIDs.push_back(hgui::TaskManager::program(std::chrono::milliseconds(250), [this] { spawn_meteors(); }));
}
...
hgui::point Wave::get_random_position(const bool onlyOnTop)
{
	static std::random_device rd;
	static std::mt19937 gen(rd());
	static std::uniform_int_distribution randomRanges(0, 1);
	if (onlyOnTop)
		return {m_spawnRanges[3][0](gen), m_spawnRanges[3][1](gen)};
	if (randomRanges(gen))
