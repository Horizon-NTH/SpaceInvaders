...
Devastator::Devastator(const Player& player) :
	Player(player, "devastator",
		player.get_level() == 1u ? std::make_tuple(1u, 1u, std::make_pair(std::chrono::milliseconds(4000), hgui::Timer())) : player.get_level() == 2u ? std::make_tuple(1u, 1u, std::make_pair(std::chrono::milliseconds(3500), hgui::Timer())) : std::make_tuple(2u, 1u, std::make_pair(std::chrono::milliseconds(3000), hgui::Timer())))
{
}
...
void Devastator::level_up()
{
	Player::level_up();
	if (m_level == 2u)
	{
		m_weaponStats = std::make_tuple(1u, 1u, std::make_pair(std::chrono::milliseconds(3500), hgui::Timer()));
	}
	else if (m_level == 3u)
	{
		m_weaponStats = std::make_tuple(2u, 1u, std::make_pair(std::chrono::milliseconds(3000), hgui::Timer()));
	}
}
